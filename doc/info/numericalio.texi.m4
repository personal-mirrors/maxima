...
@node Introduction to numericalio, Functions and Variables for plain-text input and output, numericalio-pkg, numericalio-pkg
...
@anchor{read_hash_table}
@deffn {Function} read_hash_table @
@fname{read_hash_table} (@var{S}, @var{A}) @
@fname{read_hash_table} (@var{S}, @var{A}, @var{separator_flag})

Reads the source @var{S} and returns its entire content as a @mrefdot{hashed array}
The source @var{S} may be a file name or a stream.

@code{read_hash_table} treats the first item on each line as a hash key,
