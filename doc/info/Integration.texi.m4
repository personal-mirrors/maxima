...
@c ev (f_1 (7), nouns);
@c /* Note parentheses around integrate(...) here */
...
@item @mref{quad_qawo}
@ifnottex
Integration of @math{cos(omega x) f(x)} or @math{sin(omega x) f(x)} over a
finite interval, where @math{omega} is a constant.
@end ifnottex
@tex
...
@item @mref{quad_qaws}
@ifnottex
Integration of @math{w(x) f(x)} over a finite interval @math{[a, b]}, where
@math{w} is a function of the form @math{(x - a)^alpha (b - x)^beta v(x)} and
@math{v(x)} is 1 or @math{log(x - a)} or @math{log(b - x)} or
@math{log(x - a) log(b - x)}, and @math{alpha > -1} and @math{beta > -1}.
@end ifnottex
@tex
...
function is to be integrated over the interval @var{a} to @var{b}.
...
@c ===beg===
@c quad_qawc (2^(-5)*((x-1)^2+4^(-5))^(-1), x, 2, 0, 5,
                 'epsrel=1d-7);
@c integrate (2^(-alpha)*(((x-1)^2 + 4^(-alpha))*(x-2))^(-1),
      x, 0, 5);
@c ev (%, alpha=5, numer);
@c ===end===
@example
...
@c ===beg===
@c quad_qaws (1/(x+1+2^(-4)), x, -1, 1, -0.5, -0.5, 1,
                 'epsabs=1d-9);
