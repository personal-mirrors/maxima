...
sorted alphanumerically (if @mref{allsym} is @code{true}) with respect to
covariant or contravariant indices depending upon the value of @mref{flipflag}.
...
should be fully expanded. @mref{ratexpand} is the fastest way to expand
...
assignment is given to the function @mref{remcomps}.
...
code assigns @mref{kdelta} to @code{h} if @code{h} has the same number
...
@mref{defrule} and @mref{applyb1} commands:
...
are more easily simplified with @mref{rename} or @mref{contract}.
...
If the metric tensor was not defined previously with @mref{imetric},
...
and should only be used if @mref{canform} is not capable of carrying out the
...
manner as in the @mref{rename} function. When @code{canform} is applied to a large
...
Also see the example under @mref{decsym}. Note: @code{canform} may not be able to
...
However, if @mref{icurvature} is entered in noun form, it can be evaluated
using @code{evundiff}:
...
also makes use of the global variable @mref{flipflag} which determines
...
in the @mref{igeodesic_coords} function refers to the metric @var{name}
...
in calculations. Specifically, the behavior of @mref{covdiff} and
@code{icurvature} is changed.

The frame is defined by two tensors: the inverse frame field (@mref{ifri},
the dual basis tetrad),
and the frame metric @mref{ifg}. The frame metric is the identity matrix for
...
If @code{itorsion_flag} is @code{false}, @mref{ikt1}
...
as a permutation of the frame bracket (@mref{ifb}) with the appropriate
...
@mref{igeowedge_flag} is @code{false} (the default), the first, "tensorial"
...
@mref{igeowedge_flag} flag, as in the following example:
...
such, this operation is also controlled by the setting of @mref{igeowedge_flag}.
For instance:
...
the regular Maxima @mref{tex} command will not produce the expected
output. You can try instead the @mref{tentex} command, which attempts
...
noun form of @mref{diff} taken with respect to @mref{ct_coords} subscripted
by the derivative index. The Christoffel symbols @mref{ichr1} and @mref{ichr2}
...
(@code{ug}). In addition, @mref{do} loops will be introduced summing over
all free indices so that the
transformed assignment statement can be evaluated by just doing
@mref{ev}. The following examples demonstrate the features of this
function.
