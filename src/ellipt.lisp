...
(macrolet
    ((frob (root)
       (let* ((s (string root))
	      ...
	      (f-noun (maxima-intern (concatenate 'string "%" forward)))
	      (f-verb (maxima-intern (concatenate 'string "$" forward)))
	      (inverse (concatenate 'string (string 'inverse_jacobi_) s))
	      (i-noun (maxima-intern (concatenate 'string "%" inverse)))
	      (i-verb (maxima-intern (concatenate 'string "$" inverse))))
	 `(progn
