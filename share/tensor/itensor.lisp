...
(defmfun $idummy ()                              ;Sets arguments to dummy indices
  (progn
    (incf $icounter)
    (intern (format nil "~a~d" $idummyx $icounter) "MAXIMA")))
...
(defun plusi(l)
  (cond
    ...
    ((and (isprod (caar l)) (eql (cadar l) -1)) (plusi (cdr l)))
...
(defun minusi(l)
  (cond
    ...
    (
      (and (isprod (caar l)) (eql (cadar l) -1)) 
...
(defmfun $ichr1 nargs                   ; Christoffel-symbol of the first kind
  (prog (a b c)
    (cond 
      ...
      (
        (> nargs 1)
        (and (eql 1 (length (arg 2))) (return ($ichr1 (arg 1))))
  ...
  (prog (l1 l2)
    ...
    (dolist (o (cdr e))
      (cond
        ...
        (
          (and (eq (caar o) 'mexpt) (eql (caddr o) -1))
      ...
      (cond
        ...
        (
          ...
          (prog (cov con f sgn)
            ...
            (return (cond ((and f (eql sgn -1)) (list '(mtimes) sgn c)) (t c)))
          )
        )
      )
    )
...
;;*** contract5 may return a negative result
		    (setq f (contract5 f))
(cond (
 (and (or (eq (car f) '(mtimes)) (eq (car f) '(mtimes simp))) (eql (cadr f) -1))
...
(cond ((atom x) x) (
 (and (or (equal (car x) '(mtimes)) (equal (car x) '(mtimes simp))) (eql (cadr x) -1))
...
(defun contract1 (f g)
  (prog (a b c d e cf sgn)
    ...
    (setq sgn
      (cond ((eql -1 (cadr ($canform (list '(mtimes simp) f g)))) -1) (t 1))
    )
...
;;    (setq a (contractinside a))
    ...
    (cond
      (
        ...
        (cond
          ...
          (
            ;; (not (and (eql (length a) 3) (eql (length b) 0)))
            (not (and (eql (+ (length (plusi a)) (length (minusi b))) 3) (eql (+ (length (plusi b)) (length (minusi a))) 0)))
            (return nil)
          )
        )
      )
      (
        ...
        (cond
          (
            ;;(and (eql (length a) 3) (eql (length b) 0))
            (and (eql (+ (length (plusi a)) (length (minusi b))) 3) (eql (+ (length (plusi b)) (length (minusi a))) 0))
            (setq cf
              (cons
          ...
          (
            ;;(not (and (eql (length a) 2) (eql (length b) 1)))
            (not (and (eql (+ (length (plusi a)) (length (minusi b))) 2) (eql (+ (length (plusi b)) (length (minusi a))) 1)))
            (return nil)
          )
        )
      )
    ...
    (return (cond ((eql sgn -1) (list '(mtimes) sgn f)) (t f)))
  )
)
...
;;	       nil))
         ...
         ((and
          ...
          (cond
           ((and
             (eq (caar x) $imetric)
             (eql (length (cdadr x)) 0)
             (eql (length (cdaddr x)) 2)
             (eql (length (cdddr x)) 0)
            )
            (list '(mtimes simp)
                   -1
           ...
           ((and
             (eq (caar x) $imetric)
             (eql (length (cdadr x)) 2)
             (eql (length (cdaddr x)) 0)
             (eql (length (cdddr x)) 0)
            )
            (list '(mtimes simp)
                  (list '(%determinant simp) $imetric)
          ...
          (cond
            ...
            ( ;; dg([a,b],[])/dg([],[m,n])
             (and
              ...
              (eq (caar e) $imetric)
              (eq (caar x) $imetric)
              (eql (length (cdadr e)) 2)
              (eql (length (cdaddr e)) 0)
              (eql (length (cdddr e)) 0)
              (eql (length (cdadr x)) 0)
              (eql (length (cdaddr x)) 2)
              (eql (length (cdddr x)) 0)
             )
            ...
            ( ;; dg([],[a,b])/dg([m,n],[])
             (and
              ...
              (eq (caar e) $imetric)
              (eq (caar x) $imetric)
              (eql (length (cdadr e)) 0)
              (eql (length (cdaddr e)) 2)
              (eql (length (cdddr e)) 0)
              (eql (length (cdadr x)) 2)
              (eql (length (cdaddr x)) 0)
              (eql (length (cdddr x)) 0)
            ...
            ((or
              ...
              (and ;; da([a,b],y)/da([m,n],k) with a+b=m+n, y=k
               ...
               (eql (+ (length (cdadr e)) (length (cdaddr e)))
...
(defun indices (e)
  (prog (top bottom x y p q r)
    ...
    (cond
      ...
      ((atom e))
      (
        (and (eq (caar e) 'mexpt) (eql (caddr e) -1))
...
(defmfun $components (tensor comp)
  ((lambda (len1 len2 len3 name prop)
    ...
    (cond ((and (symbolp comp) (> (f+ (f+ len1 len2) len3) 0))
           (setq prop 'carrays)
          )
...
(defun memberl (a b)
  (do ((l a (cdr l))
      ...
      ((null l) nil)
    (setq carl (car l))
    (cond ((and (symbolp carl) (member carl b :test #'equal))
           (return t)))))
...
(defmfun $changename (a b e)				;Change the name of the indexed object A to B in E
  (prog (old indspec ncov ncontr)			;INDSPEC is INDex SPECification flag
     (cond ((not (or (and (symbolp a) (setq old a))
                  (and ($listp a) (equal (length (cdr a)) 3)
                     (symbolp (setq old (cadr a)))
                     (fixnump (setq ncov (caddr a)))
                     (fixnump (setq ncontr (cadddr a)))
                     (setq indspec t))))
