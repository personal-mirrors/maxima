...
(defmfun $aberth_roots (expr)
  "-- Function: aberth_roots 
    aberth_roots(<expr>)
    aberth_roots(<eqn>)

  Computes numerical approximations of the real and complex roots of
  the polynomial <expr> or the polynomial equation <eqn> of one
  variable.

  The flag 'polyfactor' when 'true' causes 'aberth_roots' return the
  result as a product of the form 'c * product(x-r[k], k, 1, n), where
  `n` is the degree of the polynomial and `r[k]` is the 'k'-th root of
  the polynomial.

  'aberth_roots' rejects non-polynomials.  It requires that the numerator
  after 'rat''ing should be a polynomial, and it requires that the
  denominator be at most a complex number.

  The algorithm is Aberth's method (https://en.wikipedia.org/wiki/Aberth_method).  

  The implementation doesn't take advantage of the fact that real
  polynomials have complex conjugate roots.  Hence the computed roots
  will not have exactly complex conjuate roots.  They will be close,
  however.

  Multiply roots makes it more difficult for the algorithm to converge.

  The variable 'aberth_debug_level' can be set to a non-zero value to
  produce some debugging information as the algorithm proceeds.
  Higher levels produce more output.

  The variable 'aberth_use_roots_for_convergence', defaulting to
  'false', controls how convergence is determined.  When true, the
  algorithm assumes convergence of the computed updates to each root
  is small relative to the absolute value of the root.  Otherwise, if
  the value of the polynomial is close enough to zero (within
  estimated rounding error), for all the roots, the convergence is
  assumed.

  The variable 'aberth_max_iterations', defaulting to 100, controls
  how many iterations are performed before failure is declared.

  The variable 'aberth_laguerre_refine', defaulting to 'true', uses
  Laguerre's algorithm to refine each root.  The number of iterations
  is controlled by 'aberth_laguerre_iterations, defaulting to 5.
  ...
  Examples:
       ...
       (%i3) for e in soln
               ...
               2.664535259100376e-15 %i + 2.664535259100376e-15

               3.552713678800501e-15 - 4.121259276702213e-30 %i

               7.105427357601002e-15 - 1.064962222048364e-29 %i

               2.664535259100376e-15 - 2.664535259100376e-15 %i
       (%o3)                         done
       (%i4) polyfactor: true$
       (%i5) aberth_roots (eqn);
...
(defmfun $bf_aberth_roots (expr)
  ...
  A bigfloat version of 'aberth_roots'.  All operations use bfloat
  numbers."
